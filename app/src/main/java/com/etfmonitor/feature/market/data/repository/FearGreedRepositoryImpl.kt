...
import com.etfmonitor.feature.market.data.mapper.MarketMapper.toDomainFearGreed
...
/**
 * Fear & Greed Repository 구현
 *
 * ⚠️ CRITICAL: FearGreed 3x 요청 로직 유지
 * Python 분석 과정에서 대량의 데이터 손실이 발생하므로
 * 실제로는 약 3배의 데이터를 수집하여 원하는 기간만큼 남도록 합니다.
 */
@Singleton
class FearGreedRepositoryImpl @Inject constructor(
    private val localDataSource: FearGreedLocalDataSource,
    ...
    override fun getAllByMarket(market: String): Flow<List<FearGreed>> =
        localDataSource.getAllByMarket(market).map { it.toDomainFearGreed() }

    override fun getRecentByMarket(market: String, limit: Int): Flow<List<FearGreed>> =
        localDataSource.getRecentByMarket(market, limit).map { it.toDomainFearGreed() }
    ...
    override fun getByMarketAndDateRange(
        ...
        localDataSource.getByMarketAndDateRange(market, startDate, endDate).map { it.toDomainFearGreed() }

    override suspend fun getByMarketAndDate(market: String, date: String): FearGreed? =
    ...
    override suspend fun initializeFearGreed(
        ...
        try {
            // ⚠️ 3x 요청: 분석 과정의 데이터 손실을 고려하여 3배 수집, 최대 730일로 제한
            ...
            localDataSource.insertAll(fearGreedData)
            ...
            for ((index, record) in recordsList.withIndex()) {
                try {
                    if (index == 0) {
                        ...
                        logger.d("First record toString: ${record}")
                    }
