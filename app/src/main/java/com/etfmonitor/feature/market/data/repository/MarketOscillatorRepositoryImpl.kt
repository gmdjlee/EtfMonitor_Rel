 ...
 * - MarketOscillatorCalculator를 통한 Kotlin 네이티브 계산
 * - kotlin_krx로 200+ 주식 데이터 수집
    ...
    override suspend fun initializeMarketData(
        ...
        try {
            ...
            if (result.dates.isEmpty() || result.indexValues.isEmpty() || result.oscillator.isEmpty()) {
                logger.e("Empty data from oscillator calculator")
                return@withContext Result.failure(Exception("데이터를 가져오지 못했습니다"))
            }

            val dates = result.dates
